...
static uint8_t send_data(lv_disp_drv_t *disp_drv, void *bytes, size_t bytes_len)
{
    ...
    ESP_ERROR_CHECK(i2c_master_write_byte(cmd, (OLED_I2C_ADDRESS << 1) | I2C_MASTER_WRITE, true));

    for (size_t idx = 0; idx < bytes_len; idx++) {
        ESP_ERROR_CHECK(i2c_master_write_byte(cmd, data[idx], true));
    }
...
static uint8_t send_pixels(lv_disp_drv_t *disp_drv, void *color_buffer, size_t buffer_len)
{
    ...
    ESP_ERROR_CHECK(i2c_master_write_byte(cmd, OLED_CONTROL_BYTE_DATA_STREAM, true));
    ESP_ERROR_CHECK(i2c_master_write(cmd, (uint8_t *) color_buffer, buffer_len, true));
    ESP_ERROR_CHECK(i2c_master_stop(cmd));
