...
#define VALUE_LIMIT_MAX(x, max) (((x) > max) ? max : (x))
#define VALUE_MAP(x, s_min, s_max, d_min, d_max) (((x) / (s_max - s_min) * (d_max - d_min)) + d_min)
...
void Axp192_ScreenBreath(uint8_t brightness) {
    brightness = VALUE_LIMIT_MAX(brightness, AXP192_SCREEN_BRIGHTNESS_MAX);
    uint16_t vol = VALUE_MAP(brightness, AXP192_SCREEN_BRIGHTNESS_MIN, AXP192_SCREEN_BRIGHTNESS_MAX, AXP192_SCREEN_BRIGHTNESS_VOLT_MIN, AXP192_SCREEN_BRIGHTNESS_VOLT_MAX);
    vol         = (vol < 1800) ? 0 : (vol - 1800) / 100;
    ESP_LOGI(TAG, "Axp192_ScreenBreath() %d, %d", brightness, vol);
    uint8_t buf = Axp192_Read8Bit(AXP192_LDO23_VOLT_REG);
    Axp192_Write8Bit(AXP192_LDO23_VOLT_REG, ((buf & 0x0f) | ((uint16_t)vol << 4)));
}

void Axp192_ScreenOnOff(uint8_t state) {
    Axp192_EnableLDO2(state);
}